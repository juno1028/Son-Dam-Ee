...
# local_embeddings는 명조체나 궁서체 같은 스타일들을 저장하는 벡터. 그래서 약간 표처럼 여러개 있는 벡터. 그거를 local_embeddings라는 한 줄의 벡터로 만드는 거에요. local_embedding는 결국 style_vector를 한 줄로 만든거.
# encoded_source가 character embedding. 이게 무슨 말이냐면, 우리가 Encoder에 넣는 글자 이미지를 벡터화한거. 그래서 Decoder에 들어가는 벡터는 Encoded_source + Local_embeddings 에요. 그게 이제 한줄로 만들어진 벡터. 이게 Decoder에 들어간다. 이게 이미지를 복원한다.

class Encoder(nn.Module):
    def __init__(self, input_dim, generator_dim, embedding_num, norm_type):
        super(Encoder, self).__init__()
        self.conv2d = Conv2d(input_dim, generator_dim)
        self.encode_layer1 = EncodeLayer(generator_dim, generator_dim*2, embedding_num, norm_type) # embedding_num은 폰트의 총 갯수이지 않을까?
...
# activation, convolution layer, normalization 층 3개를 한번에 묶은게 EncodeLayer
# 정복
...
class Decoder(nn.Module):
    def __init__(self, input_dim, generator_dim, output_dim, inst_norm, embedding_num, norm_type):
        ...
        # 뒤에서 대칭되는 encoder layer와 oncatenate를 하기 때문에, output의 dimension(채널 수)은 1/2배이다.
...
#         self.decode_layer3 = DecodeLayer(generator_dim*8*2, generator_dim*8, inst_norm, embedding_num, norm_type=norm_type)
    ...
    def forward(self, encoded, encoding_layers, ids, one_hot_ids):
        # unet에서만 하는 거 : 너무 작아지고, 특징이 사라지니까 upsampling할 때, encoder에 있던 값을 활용하여 decoding을 한다.
...
class Generator(nn.Module):
    def __init__(self, enc_input_dim, dec_input_dim, dec_output_dim, generator_dim, 
        ...
        self.embeddings = Embedding(embedding_num, embedding_dim) # Embedding으로 lookup table을 만든다. embedding_num의 사이즈(폰트의 갯수)를 가지고, embedding_dim(tensor의 차원의 수)를 가지는
...
#  128 * 128 * 1
#  z = [[0, 124, 254, 231, .... ,12, 1],
#  [184, 1, 235, 12, 15, ... , 123, 5],
#  .....
#   [184, 1, 235, 12, 15, ... , 123, 5]]

#   -> normalization
# [[0, 0.534, 0.92123, ... 0.02, 0.00123],
# ]

    def forward(self, images, embedding_ids, one_hot_ids):
        # encoder를 통과시킬 때, 필요한 건 images, one_hot_ids이고, z, enc_layers를 return 한다.
        z, enc_layers = self.encoder(images, one_hot_ids)
        # font 종류의 갯수 : 27개
        lookup_tensor = embedding_ids
        # 휴먼 : 0, 나눔 : 1, 바른 : 2, .... -> local_embeddings = tensor[[0.12312, 0.2312, 0.123124]-> 휴먼, [-0.341, -0.2412, 0.1231] -> 나눔 ...이런식으로 쭉쭉]
        ...
        # z vector랑 local_embddings concatenate
        embedded = torch.cat([z, local_embeddings], 1)
        # output은 머 지금까지 들어온거 decoder 통과시키는거지
